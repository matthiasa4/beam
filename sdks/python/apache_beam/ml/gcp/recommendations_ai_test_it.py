...
class RecommendationAIIT(unittest.TestCase):
    ...
    def test_create_catalog_item(self):
        ...
        with TestPipeline(is_integration_test=True) as p:
            output = (p | 'Create data' >> beam.Create([CATALOG_ITEM]) |
                      'Create CatalogItem' >> recommendations_ai.CreateCatalogItem(project=p.get_option('project')) |
    ...
    def test_create_user_event(self):
        ...
        with TestPipeline(is_integration_test=True) as p:
            output = (p | 'Create data' >> beam.Create([USER_EVENT]) |
                      'Create UserEvent' >> recommendations_ai.WriteUserEvent(project=p.get_option('project')) |
        ...
        with TestPipeline(is_integration_test=True) as p:
            output = (p | 'Create data' >> beam.Create([USER_EVENT]) |
                      'Predict UserEvent' >> recommendations_ai.PredictUserEvent(
                          project=p.get_option('project'), placement_id="recently_viewed_default") |
