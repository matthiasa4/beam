...
class RecommendationsAICatalogItemTest(unittest.TestCase):
    ...
    def test_CreateCatalogItem(self):
        ...
        with mock.patch.object(recommendations_ai, 'get_recommendation_catalog_client', return_value=self._mock_client):
            ...
            _ = (p | "Create data" >> beam.Create([self._catalog_item]) |
                 "Create CatalogItem" >> recommendations_ai.CreateCatalogItem(project="test"))
    ...
    # ValueError: Input to 'Create UserEvents/ParDo(_BatchItems)' must be compatible with KV[Any, Any]. Found Dict[str, Union[Dict[str, str], str]].
    def test_ImportCatalogItems(self):
        expected_counter = 1
        with mock.patch.object(recommendations_ai, 'get_recommendation_catalog_client', return_value=self._mock_client):
            p = beam.Pipeline()

            _ = (p | "Create data" >> beam.Create([(self._catalog_item["id"], self._catalog_item),
                                                   (self._catalog_item["id"], self._catalog_item)]) |
                 "Create CatalogItems" >> recommendations_ai.ImportCatalogItems(project="test"))
...
class RecommendationsAIUserEventTest(unittest.TestCase):
    ...
    def test_CreateUserEvent(self):
        ...
        with mock.patch.object(recommendations_ai,
            ...
            _ = (p | "Create data" >> beam.Create([self._user_event]) |
                 "Create UserEvent" >> recommendations_ai.WriteUserEvent(project="test"))
    ...
    def test_ImportUserEvents(self):
        ...
        with mock.patch.object(recommendations_ai,
            ...
            _ = (p | "Create data" >> beam.Create([(self._user_event["user_info"]["visitor_id"], self._user_event),
                                                   (self._user_event["user_info"]["visitor_id"], self._user_event)]) |
                 "Create UserEvents" >> recommendations_ai.ImportUserEvents(project="test"))
...
class RecommendationsAIPredictTest(unittest.TestCase):
    ...
    def test_Predict(self):
        ...
        with mock.patch.object(recommendations_ai,
            ...
            _ = (p | "Create data" >> beam.Create([self._user_event]) | "Prediction UserEvents" >>
                 recommendations_ai.PredictUserEvent(project="test", placement_id="recently_viewed_default"))
