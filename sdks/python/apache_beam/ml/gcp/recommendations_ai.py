...
class CreateCatalogItem(PTransform):
    ...
    def __init__(self, project=None, retry=None, timeout=120, metadata=None, catalog_name="default_catalog"):
        ...
        Args:
            ...
            retry (google.api_core.retry.Retry): Optional. Designation of what errors, if any, should be retried.
    ...
    def expand(self, pcoll):
        ...
        return pcoll | ParDo(
            _CreateCatalogItemFn(self.project, self.retry, self.timeout, self.metadata, self.catalog_name))
...
class _CreateCatalogItemFn(DoFn):
    ...
    def process(self, element):
        ...
        try:
            created_catalog_item = self._client.create_catalog_item(request=request,
                                                                    retry=self.retry,
            ...
            # yield MessageToDict(created_catalog_item)
        except Exception:
            yield pvalue.TaggedOutput(self.FAILED_CATALOG_ITEMS, recommendationengine.CatalogItem.to_dict(catalog_item))
            # yield MessageToDict(created_catalog_item)


class _BatchItems(DoFn):
    """DoFn that batches up CatalogItems to reduce the number of requests made to the Recommendations API.

    Maps a PCollection of KV<String, dict> to a PCollection KV<String, List<dict>>
    """
...
class ImportCatalogItems(PTransform):
    ...
    The ``PTransform returns a PCollectionTuple with PCollections of successfully and failed imported CatalogItems.
    ...
    def expand(self, pcoll):
        ...
        if self.project is None:
            raise ValueError('GCP project name needs to be specified in "project" pipeline option')
        return (pcoll | ParDo(_BatchItems(self.max_batch_size)) |
                ParDo(_ImportCatalogItemsFn(self.project, self.retry, self.timeout, self.metadata, self.catalog_name)))
...
class _ImportCatalogItemsFn(DoFn):
    ...
    def process(self, element):
        ...
        try:
            operation = self._client.import_catalog_items(request=request,
                                                          retry=self.retry,
                                                          timeout=self.timeout,
                                                          metadata=self.metadata)
...
class WriteUserEvent(PTransform):
    ...
    The ``PTransform returns a PCollectionTuple with PCollections of successfully and failed written UserEvents.
    ...
    def expand(self, pcoll):
        ...
        if self.project is None:
            raise ValueError('GCP project name needs to be specified in "project" pipeline option')
        return pcoll | ParDo(
            _WriteUserEventFn(self.project, self.retry, self.timeout, self.metadata, self.catalog_name,
                              self.event_store))
...
class ImportUserEvents(PTransform):
    ...
    The ``PTransform returns a PCollectionTuple with PCollections of successfully and failed imported UserEvents.
    ...
    def expand(self, pcoll):
        ...
        if self.project is None:
            raise ValueError('GCP project name needs to be specified in "project" pipeline option')
        return (pcoll | ParDo(_BatchItems(self.max_batch_size)) | ParDo(
            _ImportUserEventsFn(self.project, self.retry, self.timeout, self.metadata, self.catalog_name,
                                self.event_store)))
...
class PredictUserEvent(PTransform):
    ...
    def expand(self, pcoll):
        ...
        if self.project is None:
            raise ValueError('GCP project name needs to be specified in "project" pipeline option')
        return pcoll | ParDo(
            _PredictUserEventFn(self.project, self.retry, self.timeout, self.metadata, self.catalog_name,
