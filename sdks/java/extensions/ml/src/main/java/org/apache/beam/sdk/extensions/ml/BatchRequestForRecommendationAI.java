...
/** DoFn that batches up GenericJson to reduce the number of requests made to the Recommendations API. */
class BatchRequestForRecommendationAICatalogItem extends DoFn<KV<String, GenericJson>, KV<String, Iterable<GenericJson>>> {
  private static final Logger LOG = LoggerFactory.getLogger(BatchRequestForRecommendationAICatalogItem.class);

  private final Counter numberOfRowsBagged =
      Metrics.counter(BatchRequestForRecommendationAICatalogItem.class, "numberOfRowsBagged");
   ...
   */
  public BatchRequestForRecommendationAICatalogItem(Integer maxBatchSize) {
  ...
  public void onTimer(
    ...
    if (elementsBag.read().iterator().hasNext()) {
      ...
      elementsBag
          ...
          .forEach(
              element -> {
                boolean clearBuffer = currentBatchSize > maxBatchSize;
                if (clearBuffer) {
                  LOG.debug(
                      "Clear buffer of {} items, Key {}", currentBatchSize.intValue(), element.getKey());
